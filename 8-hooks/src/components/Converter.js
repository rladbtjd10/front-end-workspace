import { useState, useEffect } from "react"; 
  ...
  const Converter = () => {
      ...
      const [bool, setBool] = useState(true); //useState("초기값이들어감")
      const [text, setText] = useState("Invert");

    const change = (e) => {
      console.log(e.target.value);
      setNumber(e.target.value); //변경된값이 기재
    };
...
return (
    <>
      ...
      <div>
      <label>Minutes</label><input type="number" placeholder="Minutes" onChange={change} value={bool ? number : Math.floor(number * 60)} disabled={!bool}/>
      </div>
      <div>
  ...
  const Btn = ({ click, btnText, backgroundColor }) => {
    return(
    <button style={{backgroundColor, color:"white", padding:"10px 20px", border: "none", borderRadius: "10px",}} onClick={click}>{btnText}</button>);
  };
