...
## sup(Superscript Text), sub(Subsctipt Text)
...
- controls : 재생도구 출력 여부
- autoplay : 자동 재생여부 지정(안먹히는 브라우저도 있음)
- loop : 반복 재생여부 지정
...
- 기존 : object element, embeded element로 Flash, media player와 같은 플로그인을 사용
...
- 텍스트를 클릭해 링크를 이동하는 방법, 이미지를 클릭해 넘어가는 방법 등 존재
...
1. 각 행의 셀 개수는 동일
2. 같은 행의 있는 셀들은 높이가 동일
3. 같은 열의 있는 셀들은 길이가 동일
...
6. `<button>` : 버튼 형식의 요소 작성(input 대신 botton 사용)
...
2. **password** : 비밀번호를 입력받을 수 있다. (위에와 같이 쓸수 있음)
...
7. **botton** : 기본적으로 가진 기능이 없어 다른 기능과 연결해서 사용한다.
